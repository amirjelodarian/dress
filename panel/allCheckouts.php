...
if ($searchMode){
    ...
    if (!empty($_GET['keyword']) && !empty($_GET['orderBy'])) {
        $checkoutsResult = $checkout->allCheckoutSearch($Funcs->EnFa($_GET['keyword'],false,true),$_GET['orderBy'],$searchStartFrom,$searchRecordPerPage);
    }
}else{
...
<div class="main-col">
    ...
    <div class="panel-search">
        ...
        <select name="checkoutsOrderBy" id="checkouts-order-by" class="order-by">
            <option value="checkout_id">شماره سفارش</option>
            <option value="user_id">آیدی مشتری</option>
            <?php if($Users->isAdministrator() || $Users->isAdmin()): ?>
                <option value="delivery_user_id">آیدی مامور تحویل</option>
            <?php endif; ?>
        </select>
    </div>
    ...
    <div id="checkouts-main-result">
        ...
        if (!$searchMode){
            switch ($delivery){
                case 'delivery':
                    $deliveryCount = $DB->count('checkout','id'," WHERE delivery_id != 0");
                    ?><h3 style="color: #017BFF;font-family: IRANSansB">(<?= $deliveryCount ?>)تحویل شده ها</h3><?php
                    break;
                case 'not_delivery':
                    $notDeliveryCount = $DB->count('checkout','id'," WHERE delivery_id = 0");
                                ...
                                <div class="Bottom_Main_COMMMENT">
                                    ...
                                    <?php if($delivery == 'delivery' && $checkoutsRow['delivery_id'] !== 0):
                                            $deliveryResult = $DB->selectAll('user_id as user_id,create_at as create_at','delivery',"WHERE checkout_id = {$checkoutsRow['id']}");
                                                ...
                                                <?php endif; ?>
                                    <?php endif; ?>
                                    <br /><div class='comment-panel-btns col-xs-12 col-sm-12 col-md-12 col-lg-12'>
                                        ...
                                        <?php endif; ?>
                                        <?php if($Users->isDeliveryAgent() && $delivery == 'not_delivery' && $checkoutsRow['delivery_id'] == 0): ?>
                                            <select name="delivery" id="delivery" class="delivery">
                                                <option value="delivery_<?= $checkoutsRow['id'] ?>" <?= $Funcs->ifNotEqual('0',$checkoutsRow['delivery_id'],'selected') ?>>تحویل</option>
                                            </select><br />
                                        <?php endif; ?>
        ...
        <?php
        if ($searchMode)
            $Funcs->checkoutsSearchPagination('checkout', $Funcs->EnFa($_GET['keyword'],false,true), $_GET['orderBy'], 'id', $searchPage, $recordPerPage,'',true);
        else
...
<script>
    $('#checkouts-search').keyup(function () {
        ...
        if (checkoutsSearch != '' && checkoutsOrderBy != ''){
            ...
            $.ajax({
                url: "allCheckoutsSearchRequests.php",
                method: "post",
                dataType: "text",
                beforeSend: function() {
                    $('.loader-outside').show();
